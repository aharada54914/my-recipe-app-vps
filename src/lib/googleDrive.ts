...
async function applyBackupData(
  backup: Partial<BackupData> & { version?: number },
  options: RestoreFromGoogleDriveOptions = {},
): Promise<boolean> {
  if (!backup || typeof backup !== 'object') return false

  const optionsStrategy = options.preferencesStrategy ?? 'prefer-newer'
...
export async function restoreFromGoogleDrive(
  ...
  if (isQaGoogleToken(token)) {
    const raw = getQaGoogleDriveBackup()
    if (!raw) return false

    try {
      const backup = JSON.parse(raw) as Partial<BackupData> & { version?: number }
      return applyBackupData(backup, options)
    } catch {
      return false
    }
  }
  ...
  try {
    const backup = (await res.json()) as Partial<BackupData> & { version?: number }
    return applyBackupData(backup, options)
  } catch {
    return false
  }
...
export async function uploadQrImageToDrive(
  ...
  if (isQaGoogleToken(token)) {
    const id = `qa-drive-${fileName}-${Date.now()}`
    return {
      id,
      webViewLink: pngDataUrl,
      webContentLink: pngDataUrl,
    }
  }
