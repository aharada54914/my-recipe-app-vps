...
出力フォーマット:
{
  ...
  "nutritionPerServing": {
    "servingSizeG": 1人前重量(number, 不明なら省略),
    "energyKcal": エネルギー(number, 不明なら省略),
    "proteinG": たんぱく質(number, 不明なら省略),
    "fatG": 脂質(number, 不明なら省略),
    "carbG": 炭水化物(number, 不明なら省略),
    "sodiumMg": ナトリウム(number, 不明なら省略),
    "saltEquivalentG": 食塩相当量(number, 不明なら省略),
    "fiberG": 食物繊維(number, 不明なら省略),
    "sugarG": 糖質(number, 不明なら省略),
    "saturatedFatG": 飽和脂肪酸(number, 不明なら省略),
    "potassiumMg": カリウム(number, 不明なら省略),
    "calciumMg": カルシウム(number, 不明なら省略),
    "ironMg": 鉄(number, 不明なら省略),
    "vitaminCMg": ビタミンC(number, 不明なら省略)
  },
...
- nutritionPerServing は情報がある項目のみ出力（不明項目は無理に推定しない）
- JSONのみ出力すること`

const NUTRITION_REPAIR_PROMPT = `あなたはレシピJSON修正AIです。
与えられたレシピJSONに対して、nutritionPerServing の必須項目を必ず埋めてください。
...
- fatG
- carbG
- saltEquivalentG（または sodiumMg）

ルール:
- 既存の title/device/category/ingredients/steps は維持する
- nutritionPerServing が不足している項目だけ推定補完する
- 数値はすべて number 型
- JSONのみ出力すること`
...
export function validateParsedRecipe(data: unknown): Omit<Recipe, 'id'> {
  ...
  return {
    ...
    ...(parsed.nutritionPerServing ? { nutritionPerServing: parsed.nutritionPerServing } : {}),
    totalTimeMinutes,
  }
}

function validateParsedRecipeWithRequiredNutrition(data: unknown): Omit<Recipe, 'id'> {
  const recipe = validateParsedRecipe(data)
  const nutritionCheck = validateRequiredNutrition(recipe)
  if (!nutritionCheck.ok) {
    throw new Error(formatMissingNutritionMessage(nutritionCheck.missingFields))
  }
  return recipe
}
...
${sourceText}`
  ...
  const repairedRaw = await generateGeminiText(repairPrompt, undefined, {
    feature: 'recipe_import_url',
    enableAutoRetryEscalation: true,
  })
  const repairedJson = extractJsonObjectText(repairedRaw)
  const repairedParsed: unknown = JSON.parse(repairedJson)
  return validateParsedRecipeWithRequiredNutrition(repairedParsed)
}
...
export async function parseRecipeText(text: string, source: 'text' | 'url' = 'text'): Promise<Omit<Recipe, 'id'>> {
  ...
  try {
    return validateParsedRecipeWithRequiredNutrition(parsed)
  } catch (error) {
    const reason = error instanceof Error ? error.message : '栄養項目が不足しています'
    return repairRecipeNutrition(text, json, reason)
  }
}
...
export async function parseRecipeFromUrl(url: string): Promise<Omit<Recipe, 'id'>> {
  ...
  if (sourceSections.length === 0) {
    const jsonLdRecipe = parseFromJsonLd()
    if (!jsonLdRecipe) throw new Error('URLからレシピ情報を抽出できませんでした。')

    const repaired = await repairRecipeNutrition(
      `JSON-LD Recipe Data:\n${JSON.stringify(jsonLdRecipe, null, 2)}`,
      JSON.stringify(jsonLdRecipe, null, 2),
      'JSON-LD由来レシピに栄養項目が不足',
    )
    repaired.sourceUrl = parsedUrl.toString()
    if (data.imageUrl && !repaired.imageUrl) repaired.imageUrl = data.imageUrl
    return repaired
  }
    ...
    if (strategy === 'gemini-first') {
      ...
      if (jsonLdRecipe) {
        const repaired = await repairRecipeNutrition(
          sourceText,
          JSON.stringify(jsonLdRecipe, null, 2),
          error instanceof Error ? error.message : 'Gemini解析に失敗',
        )
        repaired.sourceUrl = parsedUrl.toString()
        if (data.imageUrl && !repaired.imageUrl) repaired.imageUrl = data.imageUrl
        return repaired
